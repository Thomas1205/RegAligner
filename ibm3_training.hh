...
//tests have shown only minor run-time differences between the two maps
//using NondefCountType = std::map<CompactAlignedSourceWords,CountStructure>;
using IBM3NondefCountType = std::unordered_map<CompactAlignedSourceWords,CountStructure,CAHash>;


class IBM3Trainer : public FertilityModelTrainer, protected IBMConstraintStates {
public:

  IBM3Trainer(const Storage1D<Math1D::Vector<uint> >& source_sentence, const LookupTable& slookup, const Storage1D<Math1D::Vector<uint> >& target_sentence,
  ...
  void train_viterbi(uint nIter, const AlignmentSetConstraints& align_constraints,
                     FertilityModelTrainerBase* prev_model = 0, const HmmWrapperBase* passed_wrapper = 0);
...
protected:
  ...
  long double alignprob_hillclimbing(const Storage1D<uint>& source, const Storage1D<uint>& target, const SingleLookupTable& lookup,
                                     const Storage1D<Math1D::Vector<double> >& fert_prob,
                                     const Math3D::Tensor<double>& distort_prob, double p_zero,
                                     Math1D::Vector<uint>& fertility, Math2D::Matrix<long double>& expansion_prob,
                                     Math2D::Matrix<long double>& swap_prob, Math1D::Vector<AlignBaseType>& alignment, uint& nIter) const;
  ...
  double par_distortion_m_step_energy(const Math1D::Vector<double>& fsingleton_count, const Math1D::Vector<double>& fspan_count, const Math1D::Vector<double>& param) const;
  ...
  void par_distortion_m_step_unconstrained(const Math3D::Tensor<double>& fsingleton_count, const Math3D::Tensor<double>& fspan_count, uint i, uint c, uint L = 5);
  ...
  //compact form
  double nondeficient_diffpar_m_step_energy(const Math3D::Tensor<double>& fsingleton_count, const Storage1D<std::vector<Math1D::Vector<ushort,uchar> > >& open_pos,
      const Storage1D<std::vector<double> >& sum_pos_count, const Math3D::Tensor<double>& param) const;

  //compact form with interpolation
  double nondeficient_m_step_energy(const double* single_pos_count, const uint J, const std::vector<double>& sum_pos_count, const double* param1,
                                    const Math1D::Vector<double>& param2, const Math1D::Vector<double>& sum1, const Math1D::Vector<double>& sum2, double lambda) const;
  ...
  //compact form
  double nondeficient_m_step_with_interpolation_core(const Math3D::Tensor<double>& single_pos_count, const std::vector<Math1D::Vector<uchar,uchar> >& open_pos,
      const std::vector<double>& weight, Math3D::Tensor<double>& param, uint i, uint c, double start_energy, bool quiet = false);

  //compact form
  double nondeficient_m_step_with_interpolation(const Math3D::Tensor<double>& single_pos_count, const std::vector<Math1D::Vector<uchar,uchar> >& open_pos,
      const std::vector<double>& count, uint i, uint c, double start_energy);
  ...
  long double alignment_prob(uint s, const Math1D::Vector<AlignBaseType>& alignment, bool with_dict = true) const;
  ...
  long double alignment_prob(const Storage1D<uint>& source, const Storage1D<uint>& target, const SingleLookupTable& lookup,
                             const Math1D::Vector<AlignBaseType>& alignment, double p_zero,
                             const Math3D::Tensor<double>* distort_prob = 0, const Storage1D<Math1D::Vector<double> >* fert_prob = 0,
