    ...
    prior_weight_(prior_weight), smoothed_l0_(smoothed_l0), l0_beta_(l0_beta), l0_fertpen_(l0_fertpen), fix_p0_(false)
{
  ...
  within_cept_prob_.resize(nTargetClasses_,2*maxJ_-1);
  ...
  for (uint s=0; s < source_sentence_.size(); s++) {
    ...
    for (uint j=0; j < curJ; j++) {
      ...
      for (uint i=0; i < curI; i++) {
        ...
        if (curJ <= 25 || nSourceClasses_ == 1 || nTargetClasses_ == 1) {
    ...
    if (inter_distort_count[J].xDim() > class1 && inter_distort_count[J].yDim() > class2) {
      ...
      for (int j1=0; j1 < J; j1++) {
        ...
        for (int j2=0; j2 < J; j2++) {
          ...
          energy -= cur_count(j2,j1) * std::log( cur_param / sum);
          if (isnan(energy)) {
...
void IBM4Trainer::inter_distortion_m_step(const Storage1D<Storage2D<Math2D::Matrix<double> > >& inter_distort_count,
  ...
  for (uint iter = 1; iter <= maxIter; iter++) {
    ...
    //std::cerr << "gradient: " << ceptstart_grad << std::endl;
      ...
      if (cur_distort_count.xDim() > word_class) {
        ...
        for (int j1=0; j1 < J; j1++) {
          ...
          for (int j2=j1+1; j2 < J; j2++) {
            ...
            within_cept_grad[j2-j1 + displacement_offset_] -= 
              cur_count / cur_param;
          }

          for (int j2=0; j2 < J; j2++) {
            within_cept_grad[j2-j1 + displacement_offset_] += count_sum / sum;
          }
        }
      }
    }
	...
	for (uint d=0; d < cept_start_prob_.zDim(); d++)
	  sum += cept_start_prob_(x,y,d);
        ...
        if (sum > 1e-300) {
          ...
          //this combination did not occur in the viterbi alignments and neighbors
    ...
    if (fertility[i] > 0) {
      ...
      if (prev_cept_center != -1) {
        ...
        if (cept_start_mode_ != IBM4UNIFORM) {
          ...
          //std::cerr << "size: " << cur_inter_distortion_prob(sclass,tclass).size() << std::endl;
          if (cur_inter_distortion_prob(sclass,tclass).size() == 0)
            par2nonpar_inter_distortion(curJ,sclass,tclass);

          //std::cerr << "ap: sclass " << sclass << ", tclass " << tclass << ", prob: " 
          //          << cur_inter_distortion_prob(sclass,tclass)(first_j,prev_cept_center) << std::endl;
    ...
    if (aligned_source_words[i].size() > 0) {
      ...
      if (prev_cept_center != -1) {
        ...
        if (cept_start_mode_ != IBM4UNIFORM) {
          ...
          //std::cerr << "dp: sclass " << sclass << ", tclass " << tclass << ", prob: " 
          //          << cur_inter_distortion_prob(sclass,tclass)(first_j,prev_cept_center) << std::endl;
  ...
  while (true) {    
    ...
    for (uint j=0; j < curJ; j++) {
	  ...
	  //END_EXPERIMENTAL
	      ...
	      //std::cerr << "cand_next_i: " << cand_next_i << std::endl;
              ...
              if (fertility[cand_aj] > 0) {
                ...
                if (insert_pos == 0) {
                  ...
                  //std::cerr << "including prob. " << cur_intra_distortion_prob(tclass,j,aligned_source_words[cand_aj][insert_pos-1]) << std::endl;
                }
	    ...
	    if (aj != 0)
	      leaving_prob = dict_[prev_ti][lookup(j,aj-1)];
	    else
...
void IBM4Trainer::train_unconstrained(uint nIter, IBM3Trainer* ibm3) {
  ...
  IBM4WithinCeptModel fwithincept_count(within_cept_prob_.xDim(),2*maxJ_-1,MAKENAME(fwithincept_count));
        ...
        if (fertility[i] > 0) {
          ...
          if (cur_prev_cept >= 0) {
            ...
            if (inter_distort_count[curJ](sclass,tclass).size() == 0)
              inter_distort_count[curJ](sclass,tclass).resize(curJ,curJ,0.0);
