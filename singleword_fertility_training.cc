...
long double IBM3Trainer::update_alignment_by_hillclimbing(const Storage1D<uint>& source, const Storage1D<uint>& target,
  ...
  for (uint j=0; j < curJ; j++) {
    ...
    if (aj == 0) {
      ...
      assert(dict_[0][cur_source[j]-1] > 0.0);
    }
    else {
      ...
      assert(dict_[cur_target[aj-1]][cur_lookup(j,aj-1)] > 0.0);
    }
  }
  ...
  while (true) {    
    ...
    for (uint j=0; j < curJ; j++) {
      ...
      //     std::cerr << " mult by " << fert_decrease_factor[aj] << " / " << dict_[t_idx][cur_lookup(j,aj-1)] << std::endl;
    ...
    double check_ratio = best_prob / alignment_prob(source,target,lookup,alignment);
...
double IBM3Trainer::compute_external_alignment(const Storage1D<uint>& source, const Storage1D<uint>& target,
  ...
  //std::cerr << "compute external alignment, J: " << J << ", I: " << I << std::endl;
  ...
  //std::cerr << "distortion prob: " << distortion_prob_[J-1] << std::endl;
  ...
  if (use_ilp) {
    ...
    return compute_viterbi_alignment_ilp(source, target, lookup, J, alignment);
  }
  else
    return hc_prob;
  ...
  for (uint iter=1; iter <= nIter; iter++) {
    ...
    for (size_t s=0; s < source_sentence_.size(); s++) {
      ...
      if (use_ilp) {
        ...
        compute_viterbi_alignment_ilp(cur_source, cur_target, cur_lookup, curJ, alignment, 0.25);
...
IBM4Trainer::IBM4Trainer(const Storage1D<Storage1D<uint> >& source_sentence,
                         ...
                         bool no_factorial,
                         IBM4CeptStartMode cept_start_mode, bool smoothed_l0, double l0_beta)
  : FertilityModelTrainer(source_sentence,slookup,target_sentence,dict,wcooc,
                          nSourceWords,nTargetWords,sure_ref_alignments,possible_ref_alignments),
    ...
    prior_weight_(prior_weight), smoothed_l0_(smoothed_l0), l0_beta_(l0_beta)
{
  ...
  for (uint J=1; J <= maxJ_; J++) {
    if (seenJs.find(J) != seenJs.end()) {
      ...
      inter_distortion_prob_[J](0,0).resize(J,J,0.0);
      intra_distortion_prob_[J].resize(1,J,J,0.0);

      for (int j1=0; j1 < (int) J; j1++) {
        for (int j2=0; j2 < (int) J; j2++) {
          inter_distortion_prob_[J](0,0)(j2,j1) = cept_start_prob_(0,0,j2-j1+displacement_offset_);
          intra_distortion_prob_[J](0,j2,j1) = within_cept_prob_(0,j2-j1+displacement_offset_);
        }
      }
    }
  }
...
long double IBM4Trainer::update_alignment_by_hillclimbing(const Storage1D<uint>& source, const Storage1D<uint>& target, 
  ...
  const uint curI = cur_target.size();
  const uint curJ = source.size(); //source_sentence_[s].size();
    ...
    for (uint j=0; j < curJ; j++) {
      ...
      for (uint cand_aj = 0; cand_aj <= curI; cand_aj++) {
        ...
        if (cand_aj != aj) {
          ...
          if (aj != 0 && cand_aj != 0) {
            ...
            if (next_i != MAX_UINT &&
              ...
              if (prev_aj_fert > 1) {
                ...
                if (prev_i != MAX_UINT) {
                  //int ldiff = ((int) j) - ((int) cept_center[prev_i]);
          ...
          //   std::cerr << "nan in exp. move " << j << " -> " << cand_aj << std::endl;
