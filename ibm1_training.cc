...
//  => more efficient schemes should be possible
void dict_m_step(const SingleWordDictionary& fdict_count, 
  ...
  double energy = dict_m_step_energy(fdict_count,prior_weight,dict);

  std::cerr << "dict-m-step start-energy: " << energy << std::endl;

  SingleWordDictionary dict_grad = dict;
  SingleWordDictionary hyp_dict = dict;
  SingleWordDictionary new_dict = dict;

  Math1D::Vector<double> slack_vector(dict.size());
  for (uint i=0; i < dict_grad.size(); i++) {
    slack_vector[i] = 1.0 - dict[i].sum();
  }
  
  Math1D::Vector<double> new_slack_vector = slack_vector;
  ...
  for (uint iter=1; iter <= nIter; iter++) {
    ...
    for (uint i=0; i < dict_grad.size(); i++) {
      for (uint k=0; k < prior_weight[i].size(); k++) {
	double cur_dict_entry = std::max(1e-15, dict[i][k]);

	dict_grad[i][k] = prior_weight[i][k] - fdict_count[i][k] / cur_dict_entry;
      }
    }
    ...
    //reproject
    for (uint i=0; i < dict_grad.size(); i++) {
      projection_on_simplex_with_slack(new_dict[i].direct_access(), new_slack_vector[i], new_dict[i].size());
    }
    ...
    while (decreasing || hyp_energy > energy) {
      ...
      for (uint i=0; i < dict_grad.size(); i++) {
	for (uint k=0; k < prior_weight[i].size(); k++) {      
	  hyp_dict[i][k] = lambda * new_dict[i][k] + neg_lambda * dict[i][k];
	}
      }
      
      double new_energy = dict_m_step_energy(fdict_count,prior_weight,hyp_dict);
      //std::cerr << "lambda = " << lambda << ", hyp_energy = " << new_energy << std::endl;
...
// 	break;
    ...
    if (hyp_energy > energy) {
      std::cerr << "seems to be converged" << std::endl;
      break;
    }
    ...
    std::cerr << "iter #" << iter << ", lambda = " << best_lambda << ", energy: "  << energy << std::endl;

    for (uint i=0; i < dict_grad.size(); i++) {
      for (uint k=0; k < prior_weight[i].size(); k++) {      
	dict[i][k] = best_lambda * new_dict[i][k] + neg_best_lambda * dict[i][k];
      }
    }
